...
class CoplademController extends Controller {
    ...
    public function dashboard(){
        ...
        $lugar = Invitados::select(\DB::raw('distinct(lugar) distLug'))->groupBy('lugar')->get();
        foreach($lugar as $l){
            $l->invitados = $invitados->where('lugar',$l->distLug)->count();
            $l->invitadoIntegr = $invitados->where('lugar',$l->distLug)->where('integrante',1)->count();
            $l->invitadosFemale = $invitados->where('lugar',$l->distLug)->where('genero','F')->count();
            $l->invitadosMale = $invitados->where('lugar',$l->distLug)->where('genero','M')->count();
            $l->asistentes = $invitados->where('lugar',$l->distLug)->where('asistio',1)->count();
            $l->asistenteInteg = $invitados->where('lugar',$l->distLug)->where('asistio',1)->where('integrante',1)->count();
            $l->asistentesFemale = $invitados->where('lugar',$l->distLug)->where('asistio',1)->where('genero','F')->count();
            $l->asistentesMale = $invitados->where('lugar',$l->distLug)->where('asistio',1)->where('genero','M')->count();
        }//dd($lugar[0]);
        
        return view('foros.dashboard',compact('invitados','female','male','integrante','noIntegrante','asistioIntegrante','asistioNoIntegrante','asistioFemale','asistioMale','lugar'))
        ->with('aBreadCrumb',[['link' => 'active', 'label' => 'Dashboard Invitados y Asistentes']])
            ->with('activePage', 'dashboardInvitados')
            ->with('mainPage', 'invitados');
    }
}