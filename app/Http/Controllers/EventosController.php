...
class EventosController extends Controller {
    ...
    public function listado() {
        ...
        if (\Auth::User()->tipo == 1) {
            $eventos['p'] = Eventos::where('estatus', 1)->where('diario', 0)->get();
            $eventos['r'] = Eventos::where('estatus', 2)->where('diario', 0)->get();
            $eventos['na'] = Eventos::where('estatus', 4)->where('diario', 0)->get();
        } else {
            $eventos = Eventos::where('id_usuario', \Auth::User()->id)->where('diario', 0)->where('estatus', '<>', 3)->get();
        }
    ...
    public function calendario(){
        ...
        foreach ($eventos as $evento){
            $usuario = User::find($evento->id_usuario);
            $evento->direccion = Gabinete::find($usuario->id_gabinete)->direccion;
            $gabineteDatos = $usuario ? Gabinete::find($usuario->id_gabinete) : $usuario;
            $evento->color = $gabineteDatos ? $gabineteDatos->color : '#d9c0fc';
            // $evento->direccion = $gabineteDatos ? $gabineteDatos->direccion : '';
            // dd($evento, Gabinete::find(User::find($evento->id_usuario)->id_gabinete)->color);
        }
    ...
    public function getEventoDiario(Request $request) {
        ...
        $evento = Eventos::find($request->get('id'));
        $acciones = EA::where('id_evento', $evento->id)->get();
        $direccion = Gabinete::find($evento->user()->id_gabinete)->direccion;
        return view('eventos.fichaCal', [
            'evento' => $evento,
            'direccion' => $direccion,
            'acciones' => $acciones,
            'dias' => $dias,
    ...
    public function eventosDiariosUpdate(){
        if(\Auth::User()->tipo == 0)
        $eventos = Eventos::where('diario',1)->get();
        else
        $eventos = Eventos::where('id_usuario', \Auth::User()->id)->where('diario', 1)->get();
        return view('eventos.diarios', [
            'aBreadCrumb' => [['link' => 'active', 'label' => 'Actualización de Eventos Diarios']],
