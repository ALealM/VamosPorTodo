@extends('layouts.app', ['activePage' => 'showEV'])
        ...
        <table class="table-hover" style="width: 100%;" id="tablaBeneficiarios">
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($evento->ubicacion)
                            <a class="btn btn-sm blue-btn" href="{{($evento->ubicacion!=null) ? 'https://maps.google.com/?ll='.substr($evento->ubicacion, strlen(' ') + strpos($evento->ubicacion, ' '), (strlen($evento->ubicacion) - strpos($evento->ubicacion, ']'))*(-1)).','.substr($evento->ubicacion, strlen('[') + strpos($evento->ubicacion, '['), (strlen($evento->ubicacion) - strpos($evento->ubicacion, ','))*(-1)).'&z=14' : ''}}" {{($evento->ubicacion!=null) ? 'target="_blank"' : ''}} ><i class="fa fa-map-marker"></i></a>
                        @else
                            {{$evento->lugar}}
                        @endif
                    </td>
