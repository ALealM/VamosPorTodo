        ...
        <table class="table tile table-hover table-condensed table-bordered" role='grid' style="margin-top: 30px">
            ...
            <tbody>
                <tr>
                    ...
                    @foreach($meses as $mes)
                    <td style="text-align:center"><small>$</small>{{number_format($mes->monto(),2,".",",")}}</td>
                    @endforeach
                </tr>
                <tr>
                    <th style="text-align:center">%</th>
                    @foreach($meses as $mes)
                    <td style="text-align:center">{{round($mes->monto()/$totMun*100,2)}}<small>%</small></td>
                    @endforeach
                </tr>
            </tbody>
        </table>
        ...
        series: [
            {
                ...
                data: [
                    @foreach($meses as $mes)
                    {name: "{{$mes->mes}}",y: {{$mes->monto()}},porc:{{round($mes->monto()/$totMun*100,2)}}},
                    @endforeach
                ]
            }
        ]
