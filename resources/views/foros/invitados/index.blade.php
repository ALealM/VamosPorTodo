@extends('layouts.app', ['activePage' => 'indexInvitados'])
@section('content')
<style>
    .dataTables_wrapper {
...
<div style="display:inline-block; text-align:center; width:100%;">
    ...
    <div style="text-align:center;">
        ...
        <p>
            ...
            <br><span style="color: rgb(255 0 0 / 80%);">█</span> Integrantes: <span id="numInt">{{ $integrantes }}</span>
        </p>
    </div>
</div>
    ...
    <table class="table tile thead-dark dataTable table-striped responsive" role='grid' style="background-color: #292942; color:#fff;">
        <thead style="background: rgba(255,255,255,.1); color:#fff; border-radius: 5em;">
            <tr style="text-align:center;">
                ...
                @if(\Auth::User()->id == 11 || \Auth::User()->id == 16  || \Auth::User()->id == 68)
                <td style="width:10% !important; cursor:pointer"><b>ASISTENTE</b></td>
                @endif
                <td style="width:1%;">&nbsp</td>
                <td style="cursor:pointer"><b>NOMBRE</b></td>
                <!--<td><b>DEPENDENCIA</b></td>-->
                <td style="cursor:pointer"><b>PUESTO</b></td>
                <td style="cursor:pointer"><b>LUGAR</b></td>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                @if(\Auth::User()->id == 11 || \Auth::User()->id == 16  || \Auth::User()->id == 68)
                <td style="width:10% !important; text-align: center">{{($inv->asistio==1) ? 'SI' : 'NO'}}</td>
                @endif
                	@if($inv->integrante==1)
						<td style="width:1%; background-color: rgb(255 0 0 / 50%);">&nbsp</td>
						@else
						<td>&nbsp</td>
					@endif
                <td style="width:40%;">
...
<!--<td style="font-size: 12px; width:20%; text-align:center;">{{ $inv->dependencia }}</td>-->
                <td style="font-size: 12px; width:10%; text-align:center;">{{ $inv->puesto }}</td>
                <td style="width:30%; text-align:center;">{{ $inv->lugar }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<script>
    function asistencia(id){
        ...
        $.get(BASE_URL + "asistencia", {'check': check, 'id': id}, function (r) {
            $('#numAsis').empty();
            $('#numInt').empty();
    ...
    function addInvitado(){
        window.location.href = BASE_URL + "addAsistente";
    }
</script>
